"""
Qwen3-VL Thought Token Wrapper for MBEIR integration.

This module implements thought token mechanism (similar to jina_v4oa) for Qwen3-VL:
- Adds <thought_1..K> + <final> tokens to tokenizer
- Implements 4D attention mask where <final> only attends to thought tokens
- Uses <final> token embedding as query representation
- Supports asymmetric query/doc encoding (query uses thought tokens, doc doesn't)
...
that gradients can flow (for training LoRA + new token embeddings).
"""
...
class Qwen3VLThoughtWrapper(nn.Module):
    ...
    def _get_single_vector_embedding(
        ...
        print(f"[_get_single_vector_embedding] num_thought_tokens={num_thought_tokens}, use_final_token={use_final_token}")
        if use_final_token:
    ...
    def _preprocess_inputs(
        ...
        # Append thought/final tokens and truncate only raw text when needed.
        use_special_tokens = num_thought_tokens > 0 or effective_use_final_token
        if texts:
    ...
    def _forward_embeddings(
        ...
        **kwargs
        ...
        """
        # Use default model attention behavior directly.
        # This removes the custom reasoning 4D mask so <final> follows normal
        # autoregressive visibility and can attend to the global left context.
        outputs = self.model(
            input_ids=input_ids,
