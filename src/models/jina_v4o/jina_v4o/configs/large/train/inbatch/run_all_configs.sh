...
pick_latest_checkpoint() {
    ...
    if [[ -d "$ckpt_dir" ]]; then
        latest=$(ls -1t \
            "$ckpt_dir"/jina_v4o_emergency_epoch*_step*.pth \
            "$ckpt_dir"/jina_v4o_step_*.pth \
            "$ckpt_dir"/jina_v4o_epoch_*.pth 2>/dev/null | head -n1 || true)
    fi
...
for cfg in "${CONFIGS[@]}"; do
    ...
    CKPT_DIR="$UNIIR_DIR/checkpoint/jina_v4o/Large/Instruct/$EXP_SUFFIX"
    FINAL_EPOCH_CKPT="$CKPT_DIR/jina_v4o_epoch_${FINAL_EPOCH}.pth"
