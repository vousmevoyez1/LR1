...
1. **复用 candidate pool embeddings**: 使用 `/data/LR1/runs/steps_0/.../cand_pool` 的预计算embedding，不重复编码
2. **与现有代码保持一致**: 使用 `mbeir_embedder.py` 和 `mbeir_retriever.py`
3. **独立输出目录**: 每个checkpoint的结果保存在 `runs/eval_finetuned/{MT0,MT1,MT3}/`
