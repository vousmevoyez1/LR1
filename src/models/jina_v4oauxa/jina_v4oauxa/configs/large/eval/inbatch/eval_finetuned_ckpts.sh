...
# --- 基础路径配置 ---
SRC="${SRC:-/data/LR1/src}"
COMMON_DIR="${COMMON_DIR:-$SRC/common}"

UNIIR_BASE_DIR="${UNIIR_BASE_DIR:-/data/LR1}"
MBEIR_DATA_DIR="${MBEIR_DATA_DIR:-/data/M-BEIR/sub_MBEIR/}"

# 配置文件目录
MODEL="${MODEL:-jina_v4oauxa/jina_v4oauxa}"
MODEL_DIR="$SRC/models/$MODEL"
SIZE="${SIZE:-large}"
MODE="${MODE:-eval}"
EXP_NAME="${EXP_NAME:-inbatch}"
...
if [ ! -f "$COMMON_DIR/mbeir_embedder.py" ] || [ ! -f "$COMMON_DIR/mbeir_retriever.py" ]; then
    echo "ERROR: Required evaluation scripts not found under $COMMON_DIR"
    exit 1
fi
...
#"Me0T0:/data/LR1/checkpointMTO0/jina_v4o/Large/Instruct/InBatch/jina_v4o_epoch_0.pth:0:0"
CHECKPOINTS=(
     "Moldlosse2testinputT5:/data/LR1/checkpointMTO5/jina_v4o/Large/Instruct/InBatch/jina_v4o_epoch_1.pth:5:5"
    # "MtestT3:/data/LR1/checkpointMTO3/jina_v4o/Large/Instruct/InBatch/jina_v4o_epoch_2.pth:3:3"
    # "MtestT1:/data/LR1/checkpointMTO1/jina_v4o/Large/Instruct/InBatch/jina_v4o_epoch_0.pth:1:1"
    # "MtestT5:/data/LR1/checkpointMTO5/jina_v4o/Large/Instruct/InBatch/jina_v4o_epoch_2.pth:5:5"
    #"MenT5:/data/LR1/checkpoint/jina_v4o/Large/Instruct/InBatch_rs5_tt5/jina_v4o_epoch_1.pth:5:5"
    # "MT7:/data/LR1/checkpoint/jina_v4o/Large/Instruct/InBatch_rs7_tt7/jina_v4o_epoch_4.pth:7:7"
    # "MT9:/data/LR1/checkpoint/jina_v4o/Large/Instruct/InBatch_rs9_tt9/jina_v4o_epoch_4.pth:9:9"
)
...
for CKPT_CONFIG in "${CHECKPOINTS[@]}"; do
    ...
    if [[ "$CKPT_NAME" =~ ^MT([0-9]+)$ ]]; then
        PORT_OFFSET="${BASH_REMATCH[1]}"
    fi
    export MASTER_PORT=$((25700 + PORT_OFFSET))
...
echo "Total runtime: ${ELAPSED_SEC}s"
