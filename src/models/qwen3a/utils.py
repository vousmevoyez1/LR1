...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == total_iters - 1:
                progress = float(i + 1) / max(1, total_iters)
                bar_width = 24
                filled = int(bar_width * progress)
                progress_bar = "[" + ("=" * filled) + ("." * (bar_width - filled)) + "]"
                progress_pct = f"{progress * 100:6.2f}%"
                eta_seconds = iter_time.global_avg * (total_iters - i - 1)
