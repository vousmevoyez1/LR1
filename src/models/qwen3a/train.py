...
from models.qwen3a.engine import train_one_epoch, eval_engine
import models.qwen3a.utils as utils
from models.qwen3a.qwen3_thought_wrapper import Qwen3VLThoughtWrapper
from models.qwen3a.utils import freeze_base_model_keep_lora
...
def main(config):
    ...
    model.use_recursive_embedding_reasoning = bool(
        getattr(model_config, "use_recursive_embedding_reasoning", True)
    )
    if utils.is_main_process():
        print(
        ...
        print(
            "[Reasoning Mode] use_recursive_embedding_reasoning="
            f"{model.use_recursive_embedding_reasoning}"
        )
    ...
    # =================================
    # Optional legacy setup: static thought/final tokens.
    # In recursive embedding reasoning mode this is intentionally skipped.
    ...
    if (
        (not model.use_recursive_embedding_reasoning)
        and ((num_thought_tokens and num_thought_tokens > 0) or enable_final_token)
    ):
    ...
    optim_groups = []
    if len(special_params) > 0:
        optim_groups.append({"params": special_params, "lr": special_lr})
    if len(lora_params) > 0:
        optim_groups.append({"params": lora_params, "lr": trainer_config.init_lr})
    if len(optim_groups) == 0:
        raise RuntimeError("No trainable parameters found (neither LoRA nor special embeddings).")

    optimizer = torch.optim.AdamW(optim_groups, weight_decay=trainer_config.weight_decay)
