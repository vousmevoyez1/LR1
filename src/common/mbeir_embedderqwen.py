...
def _embedder_debug_enabled(model) -> bool:
    return bool(getattr(model, "debug_mode", False))


def _embedder_debug_log(model, message: str):
    if _embedder_debug_enabled(model) and dist_utils.is_main_process():
        print(f"[DEBUG][eval-embedder] {message}")


def _preview_query_text_from_batch(batch, max_chars: int = 120) -> str:
    try:
        txt_batched = batch.get("txt_batched", None)
        if txt_batched is None:
            return "<no-txt-batched>"
        sample = str(txt_batched[0])
...
def generate_embeds_and_ids_for_dataset_with_gather(model, data_loader, device, use_fp16=True):
    embedding_tensors = []
    id_list = []
    debug_print_freq = int(getattr(model, "debug_embed_print_freq", 1))
    debug_max_text_chars = int(getattr(model, "debug_max_text_chars", 120))
    ...
    for batch_idx, batch in enumerate(data_loader):
        debug_now = _embedder_debug_enabled(model) and (batch_idx % max(1, debug_print_freq) == 0)
        if debug_now:
            tensor_shapes = {k: tuple(v.shape) for k, v in batch.items() if isinstance(v, torch.Tensor)}
            _embedder_debug_log(model, f"batch_idx={batch_idx}, input_tensor_shapes={tensor_shapes}")
            _embedder_debug_log(model, f"batch_idx={batch_idx}, sample_query_text='{_preview_query_text_from_batch(batch, debug_max_text_chars)}'")
        ...
        if debug_now:
            _embedder_debug_log(
                model,
                f"batch_idx={batch_idx}, output_embeddings_shape={tuple(embeddings_batched.shape)}, "
                f"dtype={embeddings_batched.dtype}, ids_count={len(ids_list_batched)}",
            )
...
def generate_embeds_and_ids_for_dataset_with_tmp_files(
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        if debug_now:
            tensor_shapes = {k: tuple(v.shape) for k, v in batch.items() if isinstance(v, torch.Tensor)}
            _embedder_debug_log(model, f"tmp-batch_idx={batch_idx}, input_tensor_shapes={tensor_shapes}")
            _embedder_debug_log(model, f"tmp-batch_idx={batch_idx}, sample_query_text='{_preview_query_text_from_batch(batch, debug_max_text_chars)}'")
        ...
        if debug_now:
            _embedder_debug_log(
                model,
                f"tmp-batch_idx={batch_idx}, output_embeddings_shape={tuple(embeddings_batched.shape)}, "
                f"dtype={embeddings_batched.dtype}, ids_count={len(hashed_ids_list_batched)}",
            )
...
def main(config, lora_checkpoint=None):
    ...
    # Eval debug controls
    model.debug_mode = bool(getattr(config.model, "debug_mode", False))
    model.debug_embed_print_freq = int(getattr(config.model, "debug_print_freq", 1))
    model.debug_preprocess_print_freq = int(getattr(config.model, "debug_print_freq", 1))
    model.debug_max_text_chars = int(getattr(config.model, "debug_max_text_chars", 120))
    model.debug_token_preview_len = int(getattr(config.model, "debug_token_preview_len", 12))
    ...
    _embedder_debug_log(
        model,
        "effective_eval_params: "
        f"TT={getattr(model, 'mbeir_num_thought_tokens', 0)}, "
        f"FT={getattr(model, 'enable_final_token', False)}, "
        f"SYM={getattr(model, 'mbeir_symmetric_encoding', False)}, "
        f"max_token_length={getattr(model, 'max_token_length', None)}, "
        f"max_visual_pixels={getattr(model, 'max_visual_pixels', None)}, "
        f"debug_print_freq={getattr(model, 'debug_embed_print_freq', 1)}",
    )
    ...
    if lora_checkpoint is not None and os.path.exists(lora_checkpoint):
        if dist_utils.is_main_process():
            ...
            # Keep user-selected overwrite behavior.
            # Default comes from CLI (--overwrite, default=False), so existing
            # embeddings are skipped unless explicitly requested to overwrite.
            overwrite_enabled = bool(getattr(getattr(config, "embed_config", {}), "overwrite", False))
            print(f"Embed overwrite mode: {overwrite_enabled} (False => skip existing files)")
...
def parse_arguments():
    ...
    parser.add_argument("--enable_final_token", type=lambda x: x.lower() in ('true', '1', 'yes') if x is not None else None, default=None,
                        help="Enable final token for reasoning (overrides config if set)")
    ...
    parser.add_argument("--debug_mode", action="store_true",
                        help="Enable eval debug logs for input/output and preprocessing checks")
    parser.add_argument("--debug_print_freq", type=int, default=1,
                        help="Eval debug print frequency in batches")
    parser.add_argument("--debug_max_text_chars", type=int, default=120,
                        help="Max chars for debug text preview")
    parser.add_argument("--debug_token_preview_len", type=int, default=12,
                        help="Token preview length for preprocess tokenizer debug")
    parser.add_argument(
        "--overwrite",
        action="store_true",
...
if __name__ == "__main__":
    ...
    # Set enable_final_token if provided via command line (overrides config)
    if args.enable_final_token is not None:
        config.model.enable_final_token = args.enable_final_token

    # Eval debug settings (CLI has highest priority)
    config.model.debug_mode = bool(args.debug_mode) or bool(getattr(config.model, "debug_mode", False))
    config.model.debug_print_freq = int(args.debug_print_freq)
    config.model.debug_max_text_chars = int(args.debug_max_text_chars)
    config.model.debug_token_preview_len = int(args.debug_token_preview_len)
