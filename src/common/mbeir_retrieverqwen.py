...
def create_index_cpu_flat(config):
    ...
    for cand_pool_name in cand_pool_name_list:
        ...
        pbar = tqdm.tqdm(total=N, desc=f"[CPU] Indexing {cand_pool_name}", unit="vec")
...
### 这段代码用于把已生成的候选池向量构建成可快速检索的 FAISS 索引
def create_index(config):
    ...
    for cand_pool_name in cand_pool_name_list:
        ...
        print("[GPU] Loading embeddings into memory...")
        embedding_list = np.load(embed_data_path).astype("float32")
        hashed_id_list = np.load(embed_data_hashed_id_path)
        print(f"[GPU] Loaded {len(embedding_list)} embeddings with shape {embedding_list.shape}")
...
def search_index(query_embed_path, cand_index_path, batch_size=10, num_cand_to_retrieve=10):
    if not os.path.exists(query_embed_path):
        raise FileNotFoundError(f"Query embedding file not found: {query_embed_path}")
    if not os.path.exists(cand_index_path):
        raise FileNotFoundError(f"FAISS index file not found: {cand_index_path}")

    # Load the full query embeddings
    query_embeddings = np.load(query_embed_path).astype("float32")
    assert query_embeddings.ndim == 2, f"Query embeddings must be 2D, got shape={query_embeddings.shape}"
    if not np.isfinite(query_embeddings).all():
        raise ValueError(f"Query embeddings contain NaN/Inf: {query_embed_path}")
    ...
    # Dim sanity check
    if hasattr(index_cpu, "d"):
        assert (
            index_cpu.d == query_embeddings.shape[1]
        ), f"Dim mismatch: index.d={index_cpu.d} vs query_dim={query_embeddings.shape[1]}"
    ...
    pbar = tqdm.tqdm(total=num_queries, desc="[FAISS] Searching", unit="query")
...
def run_retrieval(config, query_embedder_config=None):
    ...
    print(f"[Retrieval] Using experiment.path_suffix: {expt_dir_name}")
    ...
    for (
        ...
        for dataset_name, cand_pool_name, qrel_name, metric_names in zip(
            ...
            ### 对每个 query
            for i, retrieved_indices_for_qid in tqdm.tqdm(
                enumerate(retrieved_indices), 
                total=len(retrieved_indices), 
                desc="[Recall] Computing metrics",
                unit="query"
            ):
...
def parse_arguments():
    ...
    parser.add_argument("--reason_steps", type=int, default=0, 
                        help="(Deprecated) Number of implicit reasoning steps. Used to match output directory from embedder.")
    parser.add_argument("--num_thought_tokens", type=int, default=None,
                        help="Number of thought tokens for reasoning. Used to match output directory from embedder.")
    parser.add_argument("--enable_final_token", type=lambda x: x.lower() in ('true', '1', 'yes') if x is not None else None, default=None,
                        help="Enable final token for reasoning. Used to match output directory from embedder.")
...
def main():
    ...
    # Update path_suffix to include steps for matching embedder output directories
    if steps_for_path > 0:
