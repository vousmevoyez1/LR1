...
class MBEIRDatasetBase(Dataset):
    ...
    def _get_candidate_modality_prompt(self, cand_modality, cand_txt, cand_img_path):
        """Return modality description string for candidate-side passage prompt."""
        modality = (cand_modality or "").lower().replace(" ", "")
        has_text = bool((cand_txt or "").strip())
        has_image = bool(cand_img_path)
        ...
        if modality == "text":
            return "This is a segment of text."
        if modality in multimodal_aliases:
            return "This is an image and a piece of text."

        # Fallback by actual content if modality value is missing or non-standard.
    ...
    # def _build_candidate_text_with_prompt(self, cand_txt, cand_modality, cand_img_path):
    #     """Build candidate-side text prompt with modality description."""
    #     normalized_cand_txt = format_string(cand_txt or "")
    #     modality_prompt = self._get_candidate_modality_prompt(
    #         cand_modality=cand_modality,
    #         cand_txt=normalized_cand_txt,
    #         cand_img_path=cand_img_path,
    #     )
    #     if normalized_cand_txt:
    #         return f"passage: {modality_prompt}\n{normalized_cand_txt}"
    #     return f"passage: {modality_prompt}"
    def _build_candidate_text_with_prompt(self, cand_txt, cand_modality, cand_img_path):
        """Build candidate-side text prompt with modality description."""
        normalized_cand_txt = format_string(cand_txt or "")
...
class MBEIRMainDataset(MBEIRDatasetBase):
    ...
    def __getitem__(self, index):
        ...
        pos_cand_txt = self._build_candidate_text_with_prompt(
            cand_txt=pos_cand.get("txt") or "",
            cand_modality=pos_cand_modality,
            cand_img_path=pos_cand.get("img_path", None),
        )
        ...
        if self.mode == Mode.TRAIN:
            ...
            if self.hard_neg_num > 0:
                ...
                for neg_cand_did in selected_neg_cand_id_list:
                    ...
                    neg_cand_txt = self._build_candidate_text_with_prompt(
                        cand_txt=neg_cand.get("txt") or "",
                        cand_modality=neg_cand.get("modality", None),
                        cand_img_path=neg_cand.get("img_path", None),
                    )
...
class MBEIRCandidatePoolDataset(MBEIRDatasetBase):
    ...
    def __getitem__(self, index):
        ...
        cand_txt = self._build_candidate_text_with_prompt(
            cand_txt=mbeir_cand_pool_entry.get("txt") or "",
            cand_modality=cand_modality,
            cand_img_path=img_path,
        )
