...
class MBEIRMainDataset(MBEIRDatasetBase):
    ...
    def __getitem__(self, index):
        ...
        if query_txt != "":
            query_txt_with_prompt = format_string(f"\nRetrieval Intent: {query_prompt}\nQuery: {query_txt}")
        else:
...
class MBEIRCollatorBase(object):
    ...
    def _add_candidate_text_prefix(self, txt):
        """Add a newline prefix for candidate texts before model input."""
    ...
    def __call__(self, batch):
        ...
        for inst_idx, instance in enumerate(batch):
            ...
            for instance_key in instance_keys:
                ...
                for item in items:
                    txt = item["txt"]
                    if instance_key != "query":
                        txt = self._add_candidate_text_prefix(txt)
                    img = item["img"]
